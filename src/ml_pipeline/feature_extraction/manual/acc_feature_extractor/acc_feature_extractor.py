...
class AccFeatureExtractor:
    ...
    def extract_features(self):
        ...
        for axis in ['x', 'y', 'z']:
            features[f'abs_integral_acc_{axis}'] = simps(np.abs(self.acc_data[axis]), dx=1/self.sampling_rate)
        features['abs_integral_acc_all'] = simps(np.abs(self.acc_data[['x', 'y', 'z']].values).sum(axis=1), dx=1/self.sampling_rate)
