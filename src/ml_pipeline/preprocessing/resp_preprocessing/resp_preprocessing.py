...
class RespPreprocessing:
    ...
    def plot_frequency_response(self, signal, title):
        ...
        plt.axvline(x=self.bw_lowcut, color='r', linestyle='--', label=f'Lowcut: {self.bw_lowcut} Hz')
        plt.axvline(x=self.bw_highcut, color='g', linestyle='--', label=f'Highcut: {self.bw_highcut} Hz')
