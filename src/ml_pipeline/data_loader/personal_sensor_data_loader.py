...
class PersonalSensorDataLoader:
    ...
    def _get_dataset(self, save_paths_template, include_subjects):
        config = {**self.dataset_config, "include_subjects": include_subjects}
        dataset = PerSensorDataset(self.features_path, **config)
        return dataset.preprocess_and_save_cross_validation(
            self.cv_splits, save_paths_template
        )

    def prepare_datasets(self, save_path, subject_id):
        print(f"\nPreparing dataset")
        save_paths_template = (
            save_path + f"/subject_{subject_id}" + "/fold_{split}_{type}.hdf5"
        )
        datesets_path = self._get_dataset(
            save_paths_template, include_subjects=[subject_id]
        )
    ...
    def get_data_loaders(self, datasets_path):
        ...
        for dataset_path in datasets_path:
            train_dataset = SensorDataset(
                dataset_path["train"], self.dataset_config["include_sensors"]
            )
            val_dataset = SensorDataset(
                dataset_path["val"], self.dataset_config["include_sensors"]
            )
