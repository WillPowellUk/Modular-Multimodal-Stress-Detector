...
class SlidingKVCache(nn.Module):
    """
    Standalone nn.Module containing a sliding projection of key and value cache.
    ...
    def append_projections(self, k, v):
        """
        Appends the most recent key and value projections to the cache.
        ...
        # Append the new scaled key-value pair to the cache
    ...
    def retrieve_projections(self):
        ...
        Returns:
            tuple: A tuple containing the key and value cache
        """
...
class SlidingAttnScoreCache(nn.Module):
    ...
    def __init__(
        ...
        self.register_buffer(
            "attn_score_cache",
            torch.zeros(attn_score_cache_shape, dtype=dtype),
            persistent=False,
        )
        self.register_buffer(
    ...
    def retrieve_projections(self):
        ...
        Returns:
            tuple: A tuple containing the query, key and value cache
        """
...
class MultiHeadProjection(nn.Module):
    def __init__(self, embed_dim, head_dim, num_heads):
        ...
        self.query_layers = nn.ModuleList(
            [nn.Linear(embed_dim, head_dim, bias=False) for _ in range(num_heads)]
        )
...
class SlidingCachedMultiHeadAttention(nn.Module):
    def __init__(
        self,
        embed_dim,
        num_heads,
        ...
        super(SlidingCachedMultiHeadAttention, self).__init__()
        ...
        # KV cache
        self.kv_cache_only = kv_cache_only
        if kv_cache_only:
            self.sliding_cache = SlidingKVCache(
                max_batch_size, max_seq_len, num_heads, self.head_dim, torch.float32
            )
        else:
            self.sliding_cache = SlidingAttnScoreCache(
                max_batch_size, max_seq_len, num_heads, self.head_dim, torch.float32
            )
    ...
    def attn_score_cached_scaled_dot_product_attention(self, query, key, value):
    ...
    def kv_cached_scaled_dot_product_attention(self, query, key, value):
        # Cache latest queries, keys and values which correspond to latest embedding in the sequence so that they can be reattended
        self.sliding_cache.append_projections(key, value)

        # Retreive the cached query, key and value projection tensors (old and new projected embeddings)
        k, v = self.sliding_cache.retrieve_projections()
...
class CachedSlidingAttentionEncoder(nn.Module):
    def __init__(
        ...
        self.attention = SlidingCachedMultiHeadAttention(
            d_model,
            n_head,
            max_batch_size,
            max_seq_len,
