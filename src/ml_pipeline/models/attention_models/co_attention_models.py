    ...
    def __init__(self, **kwargs):
        ...
        for modality in self.input_dims:
            modality_net = nn.ModuleDict({
                ...
                'enc1': SelfAttentionEncoder(self.embed_dim, ffn_hidden=self.hidden_dim, n_head=self.n_head, drop_prob=self.dropout),
                'flatten': nn.Flatten(),
            ...
            for j, modality2 in enumerate(modalities):
                if i != j:
                    self.cross_attention_blocks[f"{modality1}_to_{modality2}"] = nn.ModuleList(
                        [CrossAttentionEncoder(self.embed_dim, self.n_head, self.dropout) for _ in range(self.n_bcsa)]
                    )
...
class MARCONet(nn.Module):
    ...
    def __init__(self, **kwargs):
        ...
        for modality in self.input_dims:
            modality_net = nn.ModuleDict({
                'embedding': nn.Linear(self.input_dims[modality], self.embed_dim),
                'pos_enc': PositionalEncoding(self.embed_dim),
                'predictor': ModularAvgPool(self.embed_dim, self.hidden_dim, self.output_dim, self.dropout),
            })
            self.modalities[modality] = modality_net
    ...
    def forward(self, inputs):
        ...
        for modality, net in self.modalities.items():
            ...
            x = x.permute(0, 2, 1)  # Change shape to [batch_size, seq_len, features] from [batch_size, features, seq_len]
            x = net['embedding'](x)
            x = net['pos_enc'](x)
