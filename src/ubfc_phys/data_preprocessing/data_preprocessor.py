...
class UBFCPhysDataPreprocessor:
        ...
        for sid in range(1, 57):
            for task in ["T1", "T2", "T3"]:
                ...
                if os.path.exists(bvp_file) and os.path.exists(eda_file):
                    ...
                    eda_interpolated = self.interpolate_signal(
                        eda_data[0], original_freq=4, target_freq=64
                    )
    ...
    def interpolate_signal(self, signal, original_freq, target_freq):
        ...
        target_time = (
            np.arange(0, len(signal) * (target_freq / original_freq)) / target_freq
        )
        interpolator = interp1d(
            original_time, signal, kind="linear", fill_value="extrapolate"
        )
